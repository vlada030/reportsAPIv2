...
window.addEventListener('DOMContentLoaded', async (event) => {
    /// brisanje sa delayem SAMO success INFO PROZORA
    deleteSuccessMessage();
    ...
    if (elements.filterTerm) {
        ...
        [elements.filterTerm, elements.fltByName, elements.fltByCode].forEach(
            (element) => {
                element.addEventListener("input", (e) => {
                    ...
                    if (isCheckedSearchByName) {
                        filteredProducts = productsList.data.filter(
                            (product) => {
                                return product.proizvod.startsWith(
